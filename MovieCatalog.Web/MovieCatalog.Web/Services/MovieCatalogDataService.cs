...
public class MovieCatalogDataService : IMovieCatalogDataService
{
    ...
    {
        Dictionary<Genre, int> genreWithCount = new Dictionary<Genre, int>();
        List<Genre> uniqueGenres = ctx.Genres
           .Select(g => g)
           .Distinct()
           .ToList();
        for (int i = 0; i < uniqueGenres.Count; i++)
        {
            genreWithCount[uniqueGenres[i]] = ctx.TitleGenres.Where(g => g.Genre == uniqueGenres[i]).Count();
        }
    ...
    {
        return ctx.Genres.Distinct();
    }
    public async Task<List<int>> GetGenreById(int id)
    {
        var teszt = ctx.TitleGenres.Where(tg => tg.TitleId == id).ToList();
        return teszt.Select(t => t.GenreId).ToList();

    }

    public async Task<Title> GetTitleByIdAsync(int id)
    {
        return ctx.Titles.SingleOrDefault(t => t.Id == id);
    }
